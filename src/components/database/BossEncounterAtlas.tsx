...
function BossCard({ record, featured = false }: { record: BossEncounter; featured?: boolean }) {
  return <Link className={`${styles.bossCard} ${featured ? styles.bossCardFeatured : ""}`} href={`/aniimo/${record.slug}`}>
...
export function BossEncounterAtlas({ records }: { records: BossEncounter[] }) {
  ...
  return <>
    <div className={styles.bossDirectory}><p><strong>{records.length}</strong> documented encounters <span /> <b>{omega.length}</b> Omega <span /> <b>{alpha.length}</b> Alpha</p><span>Select any encounter to inspect its Aniimo record.</span></div>
