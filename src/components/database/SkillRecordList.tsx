...
export function SkillRecordList({ records }: { records: SkillRecord[] }) {
  ...
  return <>
    <div className={styles.skillGrid}>{records.slice(0, visibleCount).map((record) => <article className={styles.skillCard} key={record.id}><span className={styles.skillArtwork}>{record.iconUrl ? <Image src={record.iconUrl} alt="" width={84} height={84} /> : "✦"}</span><div className={styles.skillCardCopy}><h2>{record.name}</h2><small>{record.category}{record.cost !== "0" ? ` · Cost ${record.cost}` : ""}{record.power !== "0" ? ` · Power ${record.power}` : ""}</small><p>{record.description}</p></div><div className={styles.skillCardFooter}><span className={styles.skillUsage}>{record.owners.length === 1 ? "Used by 1 Aniimo" : `Used by ${record.owners.length} Aniimo`}</span><span className={styles.skillOwnerList}>{record.owners.map((owner) => <Link href={`/aniimo/${owner.slug}`} key={owner.slug} title={owner.name} aria-label={`Open ${owner.name}`}><span><Image src={owner.image} alt="" fill sizes="30px" /></span></Link>)}</span></div></article>)}</div>
