...
import { aniimo, databaseCategories, habitats } from "@/lib/data";
...
export default function DatabasePage() {
  const counts: Record<string, number | string> = { skills: aniimo.reduce((sum, entry) => sum + entry.skills.length, 0), traits: aniimo.reduce((sum, entry) => sum + entry.traits.length, 0), elements: new Set(aniimo.flatMap((entry) => entry.elements)).size, habitats: habitats.length, evolutions: aniimo.filter((entry) => entry.evolution.length > 1).length, forms: aniimo.reduce((sum, entry) => sum + entry.formRecords.length, 0), mobility: new Set(aniimo.flatMap((entry) => entry.mobility.map((ability) => ability.name))).size, items: 2446, materials: 101, bosses: 21, achievements: 85 };
  const categoryArt: Record<string, { src: string; alt: string; tone: string }> = {
    ...
    forms: { src: "/images/aniimo/forms/10002863.png", alt: "Regional Aniimo form", tone: "violet" },
